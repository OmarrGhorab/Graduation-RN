...
export default function NotificationListener() {
    ...
    useEffect(() => {
            ...
            if (data) {
                ...
                if (data.type === 'CHAT_MESSAGE' || data.type === 'message') {
                    ...
                    const ssePayload: SSENotification = {
                        ...
                        userImage: data.userImage || data.user_image || data.senderImage || data.sender_image,
                        messagePreview: data.messagePreview || data.body || notification.request.content.body,
                        messageType: data.chat_content_type || 'text',
                ...
                // FCM delivers all data values as strings — parse action back to object if needed
                let parsedAction = data.action;
                if (typeof parsedAction === 'string') {
                    try { parsedAction = JSON.parse(parsedAction); } catch { parsedAction = null; }
                }

                // Create notification object for cache in Unified Format
                const apiNotification: ApiNotification = {
                    // Use the real DB id from the notification service when available
                    id: data.notification_id || data.id || `push-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
                    type: data.type || 'info',
        ...
        // Listen for user interaction with notifications (tray tap)
        responseListener.current = Notifications.addNotificationResponseReceivedListener((response) => {
            ...
            navigateFromNotification(
                { type: data?.type || data?.notification_type, data, action },
                router,
                role,
            );
        });
