...
export async function submitOnboarding(data: OnboardingData): Promise<OnboardingResponse> {
    try {
        ...
        logger.log('[Auth] Submitting onboarding data with URL:', `${BASE_URL}/api/v1/onboarding`);
        ...
        logger.error('[Auth] Onboarding submission failed:', error);
...
export async function deleteProfileImage(): Promise<{ success: boolean; message: string }> {
    try {
        ...
        logger.log('[Auth] Deleting profile image with URL:', `${BASE_URL}/api/v1/auth/account/profile-image`);
        ...
        logger.error('[Auth] Profile image deletion failed:', error);
...
export async function confirmReactivation(tempToken: string): Promise<ConfirmReactivationResponse> {
    try {
        logger.log('[Auth] Confirming account reactivation with URL:', `${BASE_URL}/api/v1/auth/account/confirm-reactivation`);
        logger.log('[Auth] Using temp token (first 20 chars):', tempToken ? tempToken.substring(0, 20) + '...' : 'null');
        ...
        logger.log('[Auth] Confirm reactivation response status:', response.status);
        const responseData = await response.json();
        logger.log('[Auth] Confirm reactivation response data:', JSON.stringify(responseData, null, 2));

        if (!response.ok) {
            logger.error('[Auth] Confirm reactivation failed with status:', response.status);
            logger.error('[Auth] Error response:', responseData);
        ...
        if (responseData.user && responseData.accessToken && responseData.refreshToken) {
            logger.log('[Auth] Storing tokens after reactivation...');
        ...
        logger.error('[Auth] Account reactivation confirmation failed:', error);
